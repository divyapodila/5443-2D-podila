...
# def initialize_messenger(**kwargs):
    ...
    if direction == None:
        ...
        if None in [game, target]:
            usage(
                "`game` and `target` needed in command params to send a message."
            )
    elif direction == "listen":
        if None in [game]:
            usage(
                "`game` needed in command params to listen for messages."
            )
